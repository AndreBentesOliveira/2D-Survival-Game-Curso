...
func on_timer_timeout():
	...
	for i in anvil_count + 1:
		...
		var envil_ability = anvil_ability_scene.instantiate()
		get_tree().get_first_node_in_group("foreground_layer").add_child(envil_ability)
		envil_ability.global_position = spawn_position
		envil_ability.hitbox_component.damage = BASE_DAMAGE
